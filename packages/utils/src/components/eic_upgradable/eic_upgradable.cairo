...
/// It exposes only internal logic and no access control — the contract wraps `upgrade` in its own
/// guarded entrypoint.
#[starknet::component]
pub mod EICUpgradableComponent {
    ...
    /// Emitted when the contract's class hash is replaced.
    ...
    > of InternalTrait<TContractState> {
        /// Replaces the class hash with `new_class_hash`, emitting `Upgraded`. If `eic_data` is
        /// `Some((class_hash, calldata))`, that EIC is `library_call`ed (in this contract's storage
        /// context) first, to migrate storage / interfaces. No authorization check — guard it.
        fn upgrade(
