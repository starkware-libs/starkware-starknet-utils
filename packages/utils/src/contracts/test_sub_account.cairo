...
mod SubAccountTests {
    ...
    fn deploy_sub_account() -> ISubAccountDispatcher {
        ...
        // The constructor sets the controller to the deployer (caller), so cheat the caller
        // address of the to-be-deployed contract to CONTROLLER before deploying.
        let contract_address = contract.precalculate_address(@array![]);
        cheat_caller_address_once(:contract_address, caller_address: CONTROLLER);
    ...
    fn test_controller() {
        ...
        assert!(sub_account.controller() == CONTROLLER);
    }
    ...
    fn test_execute_single_call_mutates_state() {
        ...
        cheat_caller_address_once(
            contract_address: sub_account.contract_address, caller_address: CONTROLLER,
        );
    ...
    fn test_execute_unauthorized_caller_panics() {
        ...
        match safe_sub_account.execute(array![call]) {
            ...
            Result::Err(panic_data) => {
                assert!(*panic_data.at(0) == 'SUB_ACCOUNT: NOT CONTROLLER');
            },
        }
    }
}
