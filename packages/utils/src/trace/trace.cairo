...
pub impl TraceImpl of TraceTrait {
    ...
    fn latest(self: StoragePath<Trace>) -> Result<(u64, u128), TraceErrors> {
        self._nth_from_last(1).map_err(|_e| TraceErrors::EMPTY_TRACE)
    }
...
impl TraceHelperImpl of TraceHelperTrait {
    ...
    /// Precondition: `n` > 0.
    fn _nth_from_last(self: StoragePath<Trace>, n: u64) -> Result<(u64, u128), TraceErrors> {
        let checkpoints = self.checkpoints;
        let len = checkpoints.len();
        if len < n {
            return Result::Err(TraceErrors::INDEX_OUT_OF_BOUNDS);
        }
        let checkpoint = checkpoints[len - n].read();
