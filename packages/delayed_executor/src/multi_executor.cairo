...
//! - Execution requires a configurable quorum of approvals.
...
pub mod MultiExecutor {
    ...
    /// * `acceptance_delay` - Delay for ownership transfers (see MultiOwnedComponent).
    ...
    impl DelayedExecutorImpl of IDelayedExecutor<ContractState> {
        ...
        fn get_call_set_status(self: @ContractState, call_set_key: felt252) -> CallSetStatus {
            ...
            // Has approvals but threshold not reached (timer not started).
            if allowed_time == 0 {
            ...
            if timelock_elapsed {
                // Timelock passed but quorum not reached.
                return CallSetStatus::AwaitingQuorum;
            }

            if quorum_reached {
                // Quorum reached but timelock not elapsed.
                return CallSetStatus::AwaitingTimelock;
            }

            // Threshold reached (timer started), but neither quorum nor timelock met.
