...
pub impl TraceImpl of TraceTrait {
    ...
    fn latest(self: StoragePath<Trace>) -> Result<(u64, u128), TraceErrors> {
        self._nth_from_end(0).map_err(|_e| TraceErrors::EMPTY_TRACE)
    }
    ...
    fn penultimate(self: StoragePath<Trace>) -> Result<(u64, u128), TraceErrors> {
        self._nth_from_end(1).map_err(|_e| TraceErrors::PENULTIMATE_NOT_EXIST)
    }
...
impl TraceHelperImpl of TraceHelperTrait {
    /// Returns the checkpoint at the given position from the last.
    fn _nth_from_end(self: StoragePath<Trace>, n: u64) -> Result<(u64, u128), TraceErrors> {
        let checkpoints = self.checkpoints;
        let len = checkpoints.len();
        if len <= n {
