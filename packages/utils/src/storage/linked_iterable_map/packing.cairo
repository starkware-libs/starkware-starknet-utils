...
/// Unpack value_and_flags (single felt) to value only. For efficient read (1 felt).
pub fn unpack_value_and_flags_to_value(value_and_flags: felt252) -> (u128, u32) {
    let u256 { low, high } = value_and_flags.into();
    let val_high: u32 = (high & MASK_32.into()).try_into().unwrap();
    (low, val_high)
}

/// Unpack value_and_flags (single felt) to flags only. For efficient is_deleted (1 felt).
pub fn unpack_value_and_flags_to_flags(value_and_flags: felt252) -> (bool, bool) {
    let u256 { low: _, high } = value_and_flags.into();
    let exists = (high & TWO_POW_33) != 0;
    let is_deleted = (high & TWO_POW_32.into()) != 0;
    (exists, is_deleted)
}

/// Pack only value_and_flags from value + flags (same layout as MapEntryFelt). Used to write
/// only the value slot without reading or writing the next slot.
pub fn pack_value_and_flags(value: (u128, u32), is_deleted: bool, exists: bool) -> felt252 {
    let (val_low, val_high) = value;
    let high: u128 = val_high.into()
        + if is_deleted {
            TWO_POW_32.into()
        } else {
    ...
    (u256 { low: val_low, high }).try_into().unwrap()
}
