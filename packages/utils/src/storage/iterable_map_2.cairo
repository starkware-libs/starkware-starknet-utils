...
impl StoragePathIterableMapReadAccessImpl<
    K, V, +Copy<K>, +Drop<K>, +Store<V>, +Hash<K, HashState>,
> of StorageMapReadAccess<StoragePath<IterableMap<K, V>>> {
    type Key = K;
    type Value = Option<V>;
    fn read(self: StoragePath<IterableMap<K, V>>, key: Self::Key) -> Self::Value {
        let is_exist = self._inner_map_bool.entry(key).read();
        if !is_exist {
            return Option::None;
        }
        let v = self._inner_map.entry(key).read();
        Option::Some(v)
    }
}

impl StoragePathMutableIterableMapReadAccessImpl<
    K, V, +Drop<K>, +Copy<K>, +Store<V>, +Hash<K, HashState>,
...
> of StorageMapWriteAccess<StoragePath<Mutable<IterableMap<K, V>>>> {
    ...
    fn write(self: StoragePath<Mutable<IterableMap<K, V>>>, key: Self::Key, value: Self::Value) {
        let is_exist = self._inner_map_bool.entry(key).read();
        if is_exist {
            self._inner_map.entry(key).write(value);
            return;
        }
        ...
        if len == 0 {
            ...
            self.next.entry(tail).write(key);
            self.tail.write(key);
        }
        self.len.write(len + 1);
        self._inner_map_bool.entry(key).write(true);
        self._inner_map.entry(key).write(value);
    }
}
