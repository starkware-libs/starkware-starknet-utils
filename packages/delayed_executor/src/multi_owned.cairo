...
//! 2. After `acceptance_delay` elapses, the nominee calls `accept_ownership()`.
...
pub trait IMultiOwned<TState> {
    ...
    fn get_acceptance_delay(self: @TState) -> u64;
...
pub mod MultiOwnedComponent {
    ...
    > of InternalTrait<TContractState> {
        ...
        /// * `acceptance_delay` - Time in seconds before a nominee can accept.
        ///
        /// # Panics
        ...
        fn initializer(
            ...
            assert(acceptance_delay <= MAX_ACCEPTANCE_DELAY, Errors::ACCEPTANCE_DELAY_TOO_LONG);

            self.n_owners.write(n);
            self.acceptance_delay.write(acceptance_delay);
    ...
    > of super::IMultiOwned<ComponentState<TContractState>> {
        fn is_owner(self: @ComponentState<TContractState>, account: ContractAddress) -> bool {
            ...
            assert(stored_owner == account, 'OWNER_MAP_INCONSISTENCY');
            true
        }
        ...
        fn get_acceptance_delay(self: @ComponentState<TContractState>) -> u64 {
            self.acceptance_delay.read()
        }
        ...
        fn transfer_ownership(
            ...
            if new_owner.is_non_zero() {
                ...
                let acceptance_time_val = get_block_timestamp() + self.acceptance_delay.read();
        ...
        fn accept_ownership(ref self: ComponentState<TContractState>) {
            ...
            assert(replaced_owner.is_non_zero(), Errors::NOT_DESIGNATED_OWNER);
            assert(
                self.current_to_pending.read(replaced_owner) == new_owner,
                'INCONSISTENT_REPLACEMENT',
            );
            assert(self.is_owner(replaced_owner), 'REPLACED_NOT_OWNER');
            assert(!self.is_owner(new_owner), Errors::ALREADY_OWNER);
            assert(
    ...
    > of PrivateTrait<TContractState> {
        ...
        fn _accept_ownership(
            ...
            assert(owner_index != 0, 'INVALID_OWNER_INDEX');
