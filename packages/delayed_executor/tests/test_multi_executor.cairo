...
fn test_remove_below_threshold_clears_timer() {
    ...
    // Timer should be cleared (allowed_time becomes MAX).
    let allowed_time_after = delayed.get_call_set_allowed_time(call_set_key);
    assert_eq!(allowed_time_after, core::num::traits::Bounded::<u64>::MAX);
}
...
fn test_timelock_reset_with_owner_replacement() {
    ...
    let new_allowed_time = delayed.get_call_set_allowed_time(call_set_key);
    assert_eq!(new_allowed_time, INITIAL_TIMESTAMP + ACCEPTANCE_DELAY + 1000 + EXECUTION_DELAY);
}
