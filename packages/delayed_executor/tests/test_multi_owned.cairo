...
fn deploy_multi_owned(
    owners: Span<ContractAddress>, acceptance_delay: u64,
...
fn test_initialization() {
    ...
    assert_eq!(contract.get_acceptance_delay(), ACCEPTANCE_DELAY);
...
fn test_max_acceptance_delay_boundary() {
    ...
    assert_eq!(contract.get_acceptance_delay(), MAX_ACCEPTANCE_DELAY);
}
...
#[test]
#[should_panic(expected: 'NOT_DESIGNATED_OWNER')]
fn test_accept_not_designated_fails() {
    let contract = deploy_with_three_owners();
...
mod constructor_validation {
    ...
    use starkware_delayed_executor::types::{MAX_ACCEPTANCE_DELAY, MAX_N_SIGNERS};
