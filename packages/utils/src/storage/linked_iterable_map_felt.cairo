...
// ---------------------------------------------------------------------------
//
// LinkedIterableMapFelt is a linked iterable map keyed by felt252 (or types
// that encode to felt252). It uses **head insertion**: each new insert becomes
// the new head, so iteration order is **LIFO (stack order)**. This differs from
// `LinkedIterableMap`, which appends to the **tail** (insertion order). Callers
// should not assume identical behavior between the two.
//
// **Reinsert semantics:** Reinserting a previously deleted key does **not**
...
/// Trait for checking if a key is present and not deleted.
/// Returns true iff the key is in the map and live (read returns the stored value).
/// Returns false for never-inserted or deleted keys. Use with `read()` to distinguish
/// absent/deleted from stored zero value.
pub trait LinkedIterableMapFeltExistsTrait<T, K> {
    fn exists(self: T, key: K) -> bool;
}
...
// read() returns default (e.g. 0) for missing, deleted, or zero value; use
// exists() and is_deleted() to distinguish.
...
// --- LinkedIterableMapExistsTrait ---

pub impl LinkedIterableMapFeltExistsImpl<
    K, V, +CastableFelt<K>, +Castable160<V>, +Drop<K>,
> of LinkedIterableMapFeltExistsTrait<StoragePath<LinkedIterableMapFelt<K, V>>, K> {
    fn exists(self: StoragePath<LinkedIterableMapFelt<K, V>>, key: K) -> bool {
        let value_and_flags = self._entries.entry(CastableFelt::encode(key)).value_and_flags.read();
        let (exists, is_deleted) = unpack_value_and_flags_to_flags(value_and_flags);
        exists && !is_deleted
    }
...
pub impl LinkedIterableMapFeltExistsMutableImpl<
    K, V, +CastableFelt<K>, +Castable160<V>, +Drop<K>,
> of LinkedIterableMapFeltExistsTrait<StoragePath<Mutable<LinkedIterableMapFelt<K, V>>>, K> {
    fn exists(self: StoragePath<Mutable<LinkedIterableMapFelt<K, V>>>, key: K) -> bool {
        self.as_non_mut().exists(key)
    }
}

pub impl LinkedIterableMapFeltExistsGenericImpl<
    T,
    K,
    +Drop<T>,
    +Drop<K>,
    impl StorageAsPathImpl: StorageAsPath<T>,
    impl StoragePathImpl: LinkedIterableMapFeltExistsTrait<
        StoragePath<StorageAsPathImpl::Value>, K,
    >,
> of LinkedIterableMapFeltExistsTrait<T, K> {
    fn exists(self: T, key: K) -> bool {
        self.as_path().exists(key)
    }
}
